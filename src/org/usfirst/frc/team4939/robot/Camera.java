...
public class Camera {
	...
	private final boolean VisionEngage = false;
	public Object imgLock = new Object();
	private Thread visionThread;
	private LiftTracker pipeline;
	...
	public void enableVisionThread() {
		pipeline = new LiftTracker();
		UsbCamera camera = CameraServer.getInstance().startAutomaticCapture();
		camera.setResolution(640, 480);
		camera.setFPS(15);
		
		final CvSink cvSink = CameraServer.getInstance().getVideo(); //capture mats from camera
		final CvSource outputStream = CameraServer.getInstance().putVideo("Stream", 480, 360); //send steam to CameraServer
		final Mat mat = new Mat(); //define mat in order to reuse it
	...
	// add this function in Robot.java and add oi buttons 
	public void control() {
		if(VisionEngage){  						// change to driver controller buttons
			enableProcessing();
